...
model_name = "./models/vocs_audio_" + y_test_structure + "_v" + args.model_version
...
class LossHistory(keras.callbacks.Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		...
		path = "./csv/performance_vocs_audio_" + y_test_structure + "_v" + args.model_version + ".csv"
...
def create_datasets():
	...
	for file in os.listdir(directory):
		...
		limit = min(features.shape[0], vocs.shape[0])

		if film not in films.keys():
			films[film] = [features[:limit], vocs[:limit]]
		else:
			films[film][0] = np.concatenate((films[film][0], features[:limit]))
		...
		if y_test_structure == 'l20':
			...
			if film not in screenings_out:
				...
				if 'x_test' not in locals():
					x_test = features[:limit]
					y_test = vocs[:limit]
				else:
					x_test = np.concatenate((x_test, features[:limit]))
				...
				if 'x_train' not in locals():
					...
					x_train = np.concatenate((x_train, features[:limit]))
